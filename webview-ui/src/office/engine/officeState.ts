...
export class OfficeState {
  ...
  reassignSeat(agentId: number, seatId: string): void {
    ...
    if (!ch) { console.warn(`[reassignSeat] No character ${agentId}`); return }
    // Unassign old seat
    if (ch.seatId) {
    ...
    // Assign new seat
    const seat = this.seats.get(seatId)
    if (!seat) { console.warn(`[reassignSeat] No seat '${seatId}'`); return }
    if (seat.assigned) { console.warn(`[reassignSeat] Seat '${seatId}' already assigned`); return }
    console.log(`[reassignSeat-v3] Moving agent ${agentId} from (${ch.tileCol},${ch.tileRow}) seatId=${ch.seatId} to seat ${seatId} at (${seat.seatCol},${seat.seatRow})`)

    // Save old seat key BEFORE changing seatId
    const oldSeatKey = this.ownSeatKey(ch)
    console.log(`[reassignSeat-v3] oldSeatKey=${oldSeatKey}, charPos=(${ch.tileCol},${ch.tileRow}), blockedTiles has oldKey=${oldSeatKey ? this.blockedTiles.has(oldSeatKey) : 'n/a'}, has charPos=${this.blockedTiles.has(`${ch.tileCol},${ch.tileRow}`)}`)

    seat.assigned = true
    ch.seatId = seatId

    // Unblock BOTH old seat (where character is) AND new seat (destination)
    const newSeatKey = `${seat.seatCol},${seat.seatRow}`
    const oldWasBlocked = oldSeatKey ? this.blockedTiles.has(oldSeatKey) : false
    const newWasBlocked = this.blockedTiles.has(newSeatKey)
    if (oldWasBlocked && oldSeatKey) this.blockedTiles.delete(oldSeatKey)
    if (newWasBlocked) this.blockedTiles.delete(newSeatKey)

    console.log(`[reassignSeat-v3] BEFORE findPath: start blocked=${this.blockedTiles.has(`${ch.tileCol},${ch.tileRow}`)}, end blocked=${this.blockedTiles.has(newSeatKey)}, oldRemoved=${oldWasBlocked}, newRemoved=${newWasBlocked}`)

    const path = findPath(ch.tileCol, ch.tileRow, seat.seatCol, seat.seatRow, this.tileMap, this.blockedTiles)

    if (oldWasBlocked && oldSeatKey) this.blockedTiles.add(oldSeatKey)
    if (newWasBlocked) this.blockedTiles.add(newSeatKey)

    console.log(`[reassignSeat-v3] Path: ${path.length} steps`)
    if (path.length === 0) {
      // Debug: detailed pathfinding diagnostics
      const startBlocked = this.blockedTiles.has(`${ch.tileCol},${ch.tileRow}`)
      const endBlocked = this.blockedTiles.has(`${seat.seatCol},${seat.seatRow}`)
      const startTile = this.tileMap[ch.tileRow]?.[ch.tileCol]
      const endTile = this.tileMap[seat.seatRow]?.[seat.seatCol]

      // Check neighbors of start
      const neighbors = [[0,-1],[0,1],[-1,0],[1,0]].map(([dc,dr]) => {
        const c = ch.tileCol+dc, r = ch.tileRow+dr
        const tile = this.tileMap[r]?.[c]
        const blocked = this.blockedTiles.has(`${c},${r}`)
        return `(${c},${r}) tile=${tile} blocked=${blocked}`
      })

      console.warn(`[reassignSeat] NO PATH!
  Start: (${ch.tileCol},${ch.tileRow}) tile=${startTile} blocked=${startBlocked}
  End: (${seat.seatCol},${seat.seatRow}) tile=${endTile} blocked=${endBlocked}
  Start neighbors: ${neighbors.join(', ')}
  Total blocked: ${this.blockedTiles.size} tiles`)
    } else {
      console.log(`[reassignSeat] Path found: ${path.length} steps`)
    }
    if (path.length > 0) {
      ch.path = path
      ch.moveProgress = 0
